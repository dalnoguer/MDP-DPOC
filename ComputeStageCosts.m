...
%We have P_good_pic and Prob_get_caught_state
n_control = size(controlSpace,1);
G = zeros(n_states,n_control);
    ...
    for K = 1:n_states
        if u~='p'
            ...
            elseif map(position_intended(2),position_intended(1))<0
                G(K,u_index) = pool_num_time_steps + detected_additional_time_steps*(1-(1-Prob_get_caught_state(K_intended))^pool_num_time_steps);
            else
                G(K,u_index) = 1 + detected_additional_time_steps*Prob_get_caught_state(K_intended);
            end
        elseif u == 'p'
            G(K,u_index) = P_good_pic(K)+(1-P_good_pic(K))*(1+detected_additional_time_steps*Prob_get_caught_state(K));
        end  
    end
end
end